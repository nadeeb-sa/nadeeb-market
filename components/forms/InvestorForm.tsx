...
export default function InvestorForm({ onSuccess }: { onSuccess: () => void }) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      ...
      <div>
        <label htmlFor="i-fullName" className={labelClass}>{t("fullName")}</label>
        <input id="i-fullName" {...register("fullName")} className={inputClass} />
        {errors.fullName && <p className={errorClass}>{t("required")}</p>}
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label htmlFor="i-phone" className={labelClass}>{t("phone")}</label>
          <input id="i-phone" {...register("phone")} className={inputClass} placeholder="05xxxxxxxx" dir="ltr" />
          {errors.phone && <p className={errorClass}>{t("invalidPhone")}</p>}
        </div>
        <div>
          <label htmlFor="i-email" className={labelClass}>{t("email")}</label>
          <input id="i-email" {...register("email")} type="email" className={inputClass} dir="ltr" />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label htmlFor="i-investorType" className={labelClass}>{t("investorType")}</label>
          <select id="i-investorType" {...register("investorType")} className={`${inputClass} cursor-pointer`}>
        ...
        <div>
          <label htmlFor="i-country" className={labelClass}>{t("country")}</label>
          <input id="i-country" {...register("country")} className={inputClass} />
          {errors.country && <p className={errorClass}>{t("required")}</p>}
        </div>
      </div>

      <div>
        <label htmlFor="i-interestLevel" className={labelClass}>{t("interestLevel")}</label>
        <select id="i-interestLevel" {...register("interestLevel")} className={`${inputClass} cursor-pointer`}>
          <option value="">{t("required")}</option>
      ...
      <div>
        <label htmlFor="i-notes" className={labelClass}>{t("notes")}</label>
        <textarea id="i-notes" {...register("notes")} className={inputClass} rows={3} />
      </div>
